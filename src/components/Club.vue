<template>
    <div class="w-full h-full">
        ...
        >
            <Header v-if="club" :type="'club'" :club="clubInfo" :club_id="clubInfo.no" @updateFavorites="emit('updateFavorites')"></Header>
            ...
            <a
                ...
                :href="'http://' + clubInfo.webaddress"
                >{{ clubInfo.webaddress }}</a
            >
            </div>
        </div>
        <div class="w-5/6 m-auto pb-24">
            ...
            >   
                ...
                >
                    ...
                    >
                        ...
                        <span id="league-info" class="ml-auto mr-5 text-gray-500 text-xs">
                            <span v-if="team.games.every(element => element[Object.keys(element)[0]].length < 1)">
                                <span v-if="club.content.classes.find(c => c.gClassID === team.gClassID).games.every(element => element[Object.keys(element)[0]].length < 1)">
                                    spielfrei
                                </span>
                                <span v-else>
                    ...
                    >
                        <div
                            v-for="(subTeam, index) in team.games"
                            :key="Object.keys(subTeam)[0]"
                        >   
                            ...
                            <Match
                                v-for="match in subTeam[Object.keys(subTeam)[0]]"
                                :key="match.gID"
                                :match="match"
                                :teamID="Object.keys(subTeam)[0]"
                                :teamClassID="team.gClassID"
                            ></Match>
                            <div
                                id="no-data"
                                v-show=" !subTeam[Object.keys(subTeam)[0]].length "
                                class="mb-2"
                            >
            ...
            <div v-else class="overflow-auto max-h-[50%] w-full mx-auto bg-white rounded-2xl border border-gray-100 shadow-xl p-2">
...
const fetchClubInfo = async () => {
    ...
    clubInfo.value = json[0].searchResult.list[0];
};
...
const fetchAddress = async () => {
    ...
    const json = await response.json();
    club.value.webaddress = json[clubInfo.value.no];
};
